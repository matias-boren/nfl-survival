...
class DeadlineService {
  ...
  Timer? _refreshTimer;
  final StreamController<int> _weekController = StreamController<int>.broadcast();
  ...
  DateTime _getNextThursday(DateTime date) {
    ...
    return date.add(Duration(days: daysUntilThursday == 0 ? 7 : daysUntilThursday));
  }
  
  bool isPickDeadlinePassed(int week) {
    final deadline = getPickDeadline(week);
    print('isPickDeadlinePassed: week=$week, deadline=$deadline, now=${DateTime.now()}');
    if (deadline == null) {
  ...
  bool shouldAdvanceToNextWeek() {
    ...
    final weekEnd = DateTime(2025, 9, 3).add(Duration(days: (currentWeek * 7) + 1)); // Tuesday of next week
