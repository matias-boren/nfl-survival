...
class WeeklyDataRefreshService {
  static final WeeklyDataRefreshService _instance = WeeklyDataRefreshService._internal();
  ...
  Future<void> _checkForWeekTransition() async {
    try {
      ...
      if (isWeekComplete && _currentProcessingWeek != currentWeek) {
        print('📅 Week $currentWeek is complete! Processing results and transitioning...');
  ...
  Future<void> _processCurrentWeekResults() async {
    ...
    try {
      ...
      if (completedGames.isNotEmpty) {
        print('🎯 Processing ${completedGames.length} completed games for week $currentWeek');
  ...
  Future<void> _processWeekTransition(int completedWeek) async {
    ...
    try {
      print('🚀 Starting week transition processing for week $completedWeek...');
      ...
      for (final league in allLeagues) {
        try {
          ...
          print('✅ League ${league.name}: ${summary.totalPicks} picks processed, ${summary.eliminatedUsers.length} eliminations');
        } catch (e) {
  ...
  Future<void> _processResultsForWeek(int week, List<Game> completedGames) async {
    try {
      final allLeagues = await _getAllLeagues();
  ...
  Future<bool> _isWeekComplete(int week) async {
    try {
      ...
      final allGamesComplete = games.every((game) => game.status == GameStatus.FINAL);
  ...
  Future<int> _getCurrentWeek() async {
    try {
      ...
      final now = DateTime.now();
      final seasonStart = DateTime(now.year, 9, 1); // Approximate NFL season start
  ...
  Future<void> _prepareNextWeekData(int nextWeek) async {
    try {
      ...
      final nextWeekGames = await _nflRepository!.listGames(season: 2025, week: nextWeek);
  ...
  Map<String, dynamic> getStatus() {
    return {
      ...
      'nextRefreshTime': _refreshTimer?.isActive == true ? 'Active' : 'Inactive',
    };
  }
