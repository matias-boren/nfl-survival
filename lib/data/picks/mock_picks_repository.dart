...
class MockPicksRepository implements PicksRepository {
  ...
  Future<List<Pick>> listPicks(String leagueId, int week) async {
    ...
    return _picks.where((pick) => pick.leagueId == leagueId && pick.week == week).toList();
  }
  ...
  Future<List<Pick>> getUserPicksForLeague(String userId, String leagueId) async {
    ...
    return _picks.where((pick) => 
      pick.userId == userId && pick.leagueId == leagueId
    ).toList();
  }
