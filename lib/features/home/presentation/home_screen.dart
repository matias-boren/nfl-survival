...
final nextWeekGamesProvider = FutureProvider<List<Game>>((ref) async {
  ...
  final games = await ref.read(nflRepositoryProvider).listGames(season: 2025, week: currentWeek);
  print('nextWeekGamesProvider got ${games.length} games for week $currentWeek');
  return games;
});
                  ...
                  child: Column(
                    ...
                    children: [
                                ...
                                children: [
                                  ...
                                  Consumer(
                                    builder: (context, ref, child) {
                                      ...
                                      final timeLeftAsync = ref.watch(deadlineStatusProvider(currentWeek));
                                      final timeLeft = timeLeftAsync.valueOrNull ?? 'Loading...';
                                      return Row(
                                        children: [
                      ...
                      gamesAsync.when(
                        ...
                        loading: () => const Center(child: CircularProgressIndicator()),
                        error: (e, st) => Center(child: Text('Error loading games: $e')),
                      ),
                    ],
                  ),
                ),
            ...
            Row(
              ...
              children: [
                ...
                Consumer(
                  builder: (context, ref, child) {
                    final deadlineService = ref.read(deadlineServiceProvider);
                    final timeUntilKickoff = deadlineService.getTimeUntilKickoff(game.date);
                    return Text(
                      timeUntilKickoff,
                      style: TextStyle(
                        color: Colors.blue[600], 
  ...
  String _formatGameTime(String startIso) {
    try {
      ...
      final formatter = DateTime.now().toString().contains('PM') 
          ? 'EEE, MMM d, h:mm a'
          : 'EEE, MMM d, HH:mm';
      return dateTime.toString().split(' ')[0] + ' ' + dateTime.toString().split(' ')[1].substring(0, 5);
    } catch (e) {
      return 'Time TBD';
    }
  }
